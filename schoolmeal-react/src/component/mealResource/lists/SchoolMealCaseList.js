...
function SchoolMealCaseList() {
    ...
    const totalPages = Math.ceil(schoolMealCase.length / postsPerPage); //전체 페이지 수
    ...
    // 현재 페이지의 게시물 추출
    const currentPosts = schoolMealCase.filter(item => {
        // 검색어 필터링 적용
    ...
    useEffect(() => {
        fetch(SERVER_URL + "schoolMealCases")
            ...
            .then(data => {
                setSchoolMealCase(data._embedded.schoolMealCases);
            })
    ...
    // 상세 페이지로 이동하는 함수
    const goToDetailPage = (schoolMealCase) => {
        const schoolMealCaseId = schoolMealCase.id || (schoolMealCase._links?.self?.href ? extractIdFromHref(schoolMealCase._links.self.href) : null);
        if (!schoolMealCaseId) {
            console.error("Invalid ID:", schoolMealCaseId);
            return;
        }
        navigate(`/mealResource/school-meal-case/${schoolMealCaseId}`);
    };
    ...
    return (
        <div className="meal-resource-list-container">
            <h1 className="meal-resource-title">학교급식 우수사례</h1>
            <div className="meal-resource-button-group">
                <div className="meal-resource-left-buttons">
                    {(isAdmin || isBoardAdmin) && (
                        <Button
                            variant="outlined"
                            onClick={() => navigate("/mealResource/school-meal-case/write")}
                        >
                            새 글 쓰기
                        </Button>
                    )}
            ...
            <table className="meal-resource-table">
                ...
                <tbody className="meal-resource-tbody">
                    {currentPosts.length === 0 ? (
                        ...
                        currentPosts.map((schoolMealCase, index) => {
                            // 필터링된 게시글의 역순 인덱스를 계산
                            const reversedFilteredIndex = currentPosts.length - index;

                            return (
                                <tr
                                    key={schoolMealCase.id || `schoolMealCase-${index}`}
                                    onClick={() => goToDetailPage(schoolMealCase)}
                                    style={{ cursor: "pointer" }}
                                >
                                    <td>{reversedFilteredIndex}</td> {/* 필터링 후 역순 번호 */}
                                    <td>{schoolMealCase.title}</td>
            ...
            <div className="pagination">
                <Button
                    ...
                    disabled={currentPage === 1 || schoolMealCase.length === 0}
                >
                    <HiChevronDoubleLeft />
                </Button>
                <Button
                ...
                <Button
                    ...
                    disabled={currentPage === totalPages || schoolMealCase.length === 0}
                >
                    <HiChevronRight />
                </Button>
                <Button
...
export default SchoolMealCaseList;
