...
function MealFacilityEquipmentList() {
    ...
    const totalPages = Math.ceil(mealFacilityEquipment.length / postsPerPage); //전체 페이지 수
    ...
    // 현재 페이지의 게시물 추출
    const currentPosts = mealFacilityEquipment.filter(item => {
        // 검색어 필터링 적용
    ...
    useEffect(() => {
        fetch(SERVER_URL + "mealFacilityEquipments")
            .then(response => response.json())
            .then(data => {
                setMealFacilityEquipment(data._embedded.mealFacilityEquipments);
            })
    ...
    // 상세 페이지로 이동하는 함수
    const goToDetailPage = (mealFacilityEquipment) => {
        const mealFacilityEquipmentId = mealFacilityEquipment.id || (mealFacilityEquipment._links?.self?.href ? extractIdFromHref(mealFacilityEquipment._links.self.href) : null);
        if (!mealFacilityEquipmentId) {
            console.error("Invalid ID:", mealFacilityEquipmentId);
            return;
        }
        navigate(`/mealResource/meal-facility-equipment/${mealFacilityEquipmentId}`);
    };
    ...
    return (
        <div className="meal-resource-list-container">
            <h1 className="meal-resource-title">급식 시설 및 설비</h1>
            <div className="meal-resource-button-group">
                <div className="meal-resource-left-buttons">
                    {(isAdmin || isBoardAdmin) && (
                        <Button
                            variant="outlined"
                            onClick={() => navigate("/mealResource/meal-facility-equipment/write")}
                        >
                            새 글 쓰기
                        </Button>
                    )}
            ...
            <table className="meal-resource-table">
                ...
                <tbody className="meal-resource-tbody">
                    {currentPosts.length === 0 ? (
                        ...
                        currentPosts.map((mealFacilityEquipment, index) => {
                            // 필터링된 게시글의 역순 인덱스를 계산
                            const reversedFilteredIndex = currentPosts.length - index;

                            return (
                                <tr
                                    key={mealFacilityEquipment.id || `mealFacilityEquipment-${index}`}
                                    onClick={() => goToDetailPage(mealFacilityEquipment)}
                                    style={{ cursor: "pointer" }}
                                >
                                    <td>{reversedFilteredIndex}</td> {/* 필터링 후 역순 번호 */}
                                    <td>{mealFacilityEquipment.title}</td>
                                    <td>{formatDate(mealFacilityEquipment.createdDate)}</td>
                                    <td>{mealFacilityEquipment.writer}</td>
                                    <td>
            ...
            <div className="pagination">
                <Button
                    ...
                    disabled={currentPage === 1 || mealFacilityEquipment.length === 0}
                >
                    <HiChevronDoubleLeft />
                </Button>
                <Button
                ...
                <Button
                    ...
                    disabled={currentPage === totalPages || mealFacilityEquipment.length === 0}
                >
                    <HiChevronRight />
                </Button>
                <Button
...
export default MealFacilityEquipmentList;
