...
function NutritionManageList() {
    ...
    const goToDetailPage = (nutritionManage) => {
        const hygieneId = nutritionManage.id || (nutritionManage._links?.self?.href ? extractIdFromHref(nutritionManage._links.self.href) : null);
        if (!hygieneId) {
            console.error("Invalid ID:", hygieneId);
            return;
        }
        navigate(`/mealResource/nutrition-manage/${hygieneId}`);
    };
