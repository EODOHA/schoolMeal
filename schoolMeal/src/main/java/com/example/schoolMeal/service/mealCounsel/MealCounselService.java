...
@Service
public class MealCounselService extends PathResolver {
	...
	/* 파일 업로드 경로 설정 */
	private String mealCounselPath;

	@PostConstruct
	public void init() {
		mealCounselPath = buildPath("매뉴얼 및 상담 관련 자료");

		// 저장 경로의 유효성 검사
		File saveDir = new File(mealCounselPath);
		if (!saveDir.exists() && !saveDir.mkdirs()) {
			throw new RuntimeException("저장 폴더를 생성할 수 없습니다: " + mealCounselPath);
		}
	}

	// 게시글 저장
	public void write(MealCounsel mealCounsel, MultipartFile file) {
		try {
			if (mealCounsel == null) {
				throw new IllegalArgumentException("MealCounsel 객체가 null입니다.");
			}
			...
			if (file != null && !file.isEmpty()) {
				...
				mealCounsel.setFileUrl(fileUrl); // fileUrl 설정
				mealCounsel.getFileUrlId();
			}

			// 게시글 저장
			MealCounsel savedMealCounsel = mealCounselRepository
					.save(mealCounsel);
			System.out.println("DB에 저장된 MealCounsel ID: " + savedMealCounsel.getId());
		} catch (IOException e) {
	...
	public FileUrl saveFile(MultipartFile file) throws IOException {
		...
		String filePath = mealCounselPath + File.separator + filename;
	...
	// 게시글 리스트 반환 메서드
	public List<MealCounsel> mealCounselList() {
		return mealCounselRepository.findAll();
	}

	// 특정 파일 정보 조회
	public FileUrl getFileUrlByMealCounselId(Long id) {
		// 해당 ID의 MealCounsel 조회
		...
		// MealCounsel에 연결된 FileUrl 조회
		return mealCounsel.getFileUrl(); // 파일이 없으면 null 반환
	}

	// 특정 게시글을 조회하면서 첨부 파일 정보도 함께 반환
	public MealCounsel getPostWithFileDetails(Long id) {
		try {
			MealCounsel mealCounsel = mealCounselRepository.findById(id)
					.orElseThrow(() -> new IllegalArgumentException("해당 ID의 게시글이 존재하지 않습니다: " + id));

			if (mealCounsel.getFileUrl() != null) {
				FileUrl fileUrl = fileUrlRepository.findById(mealCounsel.getFileUrl().getId()).orElse(null); // 처리
				mealCounsel.setFileUrl(fileUrl); // MealCounsel에 FileUrl 설정
			}
	...
	// 게시글 삭제
	@Transactional
	public ResponseEntity<Integer> mealCounselDelete(Long id) {
		try {
			// 1. 삭제할 MealCounsel 조회
			MealCounsel mealCounsel = mealCounselRepository.findById(id)
					.orElseThrow(() -> new IllegalArgumentException("해당 ID의 게시글이 존재하지 않습니다: " + id));

			// 2. MealCounsel에서 파일 참조 해제 (FILE_ID를 null로 설정)
			if (mealCounsel.getFileUrl() != null) {
				FileUrl fileUrl = mealCounsel.getFileUrl();
				mealCounsel.setFileUrl(null); // fileUrl 설정
				mealCounselRepository.save(mealCounsel); // DB에 반영
			...
			// 5. 게시글 삭제
			mealCounselRepository.deleteById(id);
	...
	// 게시글 수정
	@Transactional
	public void mealCounselUpdate(MealCounsel mealCounsel, MultipartFile file)
			throws IOException {
		try {
			if (file != null && !file.isEmpty()) {
				FileUrl existingFile = mealCounsel.getFileUrl(); // 기존 파일 정보 가져오기
				...
				if (existingFile != null) {
					...
					mealCounsel.setFileUrl(newFileUrl);
				}

				// 파일 정보
				if (mealCounsel.getFileUrl() != null) {
					Long fileId = mealCounsel.getFileUrl().getId();
				}
			}
	...
	private String saveFileToDisk(MultipartFile file) throws IOException {
		...
		String uploadDir = mealCounselPath;
