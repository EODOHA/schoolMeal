...
@Service
public class MealCounselHistoryService extends PathResolver {
	...
	/* 파일 업로드 경로 설정 */
	private String mealCounselHistoryPath;

	@PostConstruct
	public void init() {
		mealCounselHistoryPath = buildPath("영양상담 이력관리");

		// 저장 경로의 유효성 검사
		File saveDir = new File(mealCounselHistoryPath);
		if (!saveDir.exists() && !saveDir.mkdirs()) {
			throw new RuntimeException("저장 폴더를 생성할 수 없습니다: " + mealCounselHistoryPath);
		}
	}

	// 게시글 저장
	public void write(MealCounselHistory mealCounselHistory, MultipartFile file) {
		try {
			if (mealCounselHistory == null) {
				throw new IllegalArgumentException("MealCounselHistory 객체가 null입니다.");
			}
			...
			if (file != null && !file.isEmpty()) {
				...
				mealCounselHistory.setFileUrl(fileUrl); // fileUrl 설정
				mealCounselHistory.getFileUrlId();
			}

			// 게시글 저장
			MealCounselHistory savedMealCounselHistory = mealCounselHistoryRepository
					.save(mealCounselHistory);
			System.out.println("DB에 저장된 MealCounselHistory ID: " + savedMealCounselHistory.getId());
		} catch (IOException e) {
	...
	public FileUrl saveFile(MultipartFile file) throws IOException {
		...
		String filePath = mealCounselHistoryPath + File.separator + filename;
	...
	// 게시글 리스트 반환 메서드
	public List<MealCounselHistory> mealCounselHistoryList() {
		return mealCounselHistoryRepository.findAll();
	}

	// 특정 파일 정보 조회
	public FileUrl getFileUrlByMealCounselHistoryId(Long id) {
		// 해당 ID의 MealCounselHistory 조회
		...
		// MealCounselHistory에 연결된 FileUrl 조회
		return mealCounselHistory.getFileUrl(); // 파일이 없으면 null 반환
	}

	// 특정 게시글을 조회하면서 첨부 파일 정보도 함께 반환
	public MealCounselHistory getPostWithFileDetails(Long id) {
		try {
			MealCounselHistory mealCounselHistory = mealCounselHistoryRepository.findById(id)
					.orElseThrow(() -> new IllegalArgumentException("해당 ID의 게시글이 존재하지 않습니다: " + id));

			if (mealCounselHistory.getFileUrl() != null) {
				FileUrl fileUrl = fileUrlRepository.findById(mealCounselHistory.getFileUrl().getId()).orElse(null); // 처리
				mealCounselHistory.setFileUrl(fileUrl); // MealCounselHistory에 FileUrl 설정
			}

			return mealCounselHistory;
		} catch (Exception e) {
	...
	// 게시글 삭제
	@Transactional
	public ResponseEntity<Integer> mealCounselHistoryDelete(Long id) {
		try {
			// 1. 삭제할 MealCounselHistory 조회
			MealCounselHistory mealCounselHistory = mealCounselHistoryRepository.findById(id)
					.orElseThrow(() -> new IllegalArgumentException("해당 ID의 게시글이 존재하지 않습니다: " + id));

			// 2. MealCounselHistory에서 파일 참조 해제 (FILE_ID를 null로 설정)
			if (mealCounselHistory.getFileUrl() != null) {
				FileUrl fileUrl = mealCounselHistory.getFileUrl();
				mealCounselHistory.setFileUrl(null); // fileUrl 설정
				mealCounselHistoryRepository.save(mealCounselHistory); // DB에 반영
			...
			// 5. 게시글 삭제
			mealCounselHistoryRepository.deleteById(id);
	...
	// 게시글 수정
	@Transactional
	public void mealCounselHistoryUpdate(MealCounselHistory mealCounselHistory, MultipartFile file)
			throws IOException {
		try {
			if (file != null && !file.isEmpty()) {
				FileUrl existingFile = mealCounselHistory.getFileUrl(); // 기존 파일 정보 가져오기
				...
				if (existingFile != null) {
					...
					mealCounselHistory.setFileUrl(newFileUrl);
				}

				// 파일 정보
				if (mealCounselHistory.getFileUrl() != null) {
					Long fileId = mealCounselHistory.getFileUrl().getId();
				}
			}

			// 게시글 업데이트
			mealCounselHistoryRepository.save(mealCounselHistory);
	...
	private String saveFileToDisk(MultipartFile file) throws IOException {
		...
		String uploadDir = mealCounselHistoryPath;
