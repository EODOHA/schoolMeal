...
//	Logger logger = LoggerFactory.getLogger(SignupController.class);
	...
	public ResponseEntity<Map<String, Object>> kakaoAuth(@RequestBody Map<String, String> request) {
		String accessToken = request.get("token"); // 액세스 토큰 받기
		String memberId = request.get("memberId");
...
//		logger.debug("@@@@@@@@@@@@@@@@@@@@@@ response: {}", response);
		...
		try {
			// 카카오 액세스 토큰 확인
			boolean isVerified = memberService.verifyKakaoAndSignup(member, accessToken);

			if (isVerified) {
				response.put("success", true);
				response.put("message", "카카오 토큰 인증 성공");
				return ResponseEntity.ok(response);
			} else {
				response.put("success", false);
				response.put("message", "인증 토큰이 일치하지 않습니다.");
	...
	public ResponseEntity<Map<String, Object>> signup(@Valid @RequestBody MemberSignupDto memberSignupDto,
		...
		if (bindingResult.hasErrors()) {
			List<String> errorMessages = bindingResult.getFieldErrors().stream().map(error -> error.getDefaultMessage())
		...
		try {
			...
			// 인증 방식 설정
			String authMethod = memberSignupDto.getAuthMethod();
			member.setAuthMethod(authMethod);
